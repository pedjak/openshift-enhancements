...
granted broad permissions to avoid these failures, so the signal was rarely useful. The
trust boundary for what gets installed shifts to catalog curation: which catalogs are added
to the cluster and which channels are followed.
...
- **Increased trust surface.** The operator-controller is now a higher-value target. Any
  ...
  more impact because the controller can write any resource.
...
permission set changes on every upgrade.
